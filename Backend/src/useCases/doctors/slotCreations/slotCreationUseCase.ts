...
export class CreateSlotUseCase implements IslotCreationUseCase {
    ...
    async createSlot(data: SlotEntity): Promise<SlotDTO> {
        ...
        const checks = data.timings.map((item) => {
            this._slotRepository.findExistingSlot(data.doctorId.toString(), data.date, item.startTime, item.endTime, startOfDate, endOfDate)
        })
        const results = await Promise.all(checks)
        if (results.some(Boolean)) {
            throw new Error('Slot conflict found!');
        }
